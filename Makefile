...
rebranding:
	...
	find . \( -name "*.md" -o -name ".gitignore" -o -name "manifest.yaml" -o -name "*Service.yml" \) -type f -not -path "./gotty/*" -exec sed -i '' -e "s/gotty/$$NEW_PKG_ID/g; s/Nostr Wallet Connect/$$NEW_PKG_TITLE/g" {} +; \
...
$(PKG_ID).s9pk: manifest.yaml instructions.md nwc_icon.svg LICENSE scripts/embassy.js  docker-images/aarch64.tar docker-images/x86_64.tar
ifeq ($(ARCH),aarch64)
